...
import (
	...
	iciclegnark "github.com/ingonyama-zk/iciclegnark/curves/bw6761"
)
...
func Prove(r1cs *cs.R1CS, pk *ProvingKey, fullWitness witness.Witness, opts ...backend.ProverOption) (*groth16_bw6761.Proof, error) {
	...
	cfg := bw6761.GetDefaultMSMConfig()
	cfg.Ctx.Stream = &stream
	cfg.IsAsync = true

	outHost := make(core.HostSlice[bw6761.Projective], 1)
	...
	gerr := bw6761.Msm(wireValuesBdevice, pk.G1Device.B, &cfg, out)
	...
	gerr = bw6761.Msm(wireValuesAhost, pk.G1Device.A, &cfg, out)
	if gerr != cuda_runtime.CudaSuccess {
	...
	gerr = bw6761.Msm(h_device, pk.G1Device.Z, &cfg, out)
	if gerr != cuda_runtime.CudaSuccess {
	...
	gerr = bw6761.Msm(_wireValuesHost, pk.G1Device.K, &cfg, out)
	if gerr != cuda_runtime.CudaSuccess {
	...
	outHostG2 := make(core.HostSlice[bw6761.G2Projective], 1)
	var outG2 core.DeviceSlice
	outG2.MallocAsync(outHostG2.SizeOfElement(), outHostG2.SizeOfElement(), stream)
	gerr = bw6761.G2Msm(wireValuesBdevice, pk.G2Device.B, &cfg, outG2)
	if gerr != cuda_runtime.CudaSuccess {
...
func computeHonDevice(a, b, c []fr.Element, domain *fft.Domain, stream cuda_runtime.Stream) core.DeviceSlice {
	...
	var configCosetGen [12]uint32
	configCosetGenRaw := core.ConvertUint64ArrToUint32Arr(cosetBits[:])
	copy(configCosetGen[:], configCosetGenRaw[:12])

	cfg := bw6761.GetDefaultNttConfig()
	cfg.IsAsync = true
	cfg.Ctx.Stream = &stream
	...
	bw6761.Ntt(a_device, core.KInverse, &cfg, a_device)
	bw6761.Ntt(b_device, core.KInverse, &cfg, b_device)
	bw6761.Ntt(c_device, core.KInverse, &cfg, c_device)

	cfg.CosetGen = configCosetGen
	cfg.Ordering = core.KMN

	bw6761.Ntt(a_device, core.KForward, &cfg, a_device)
	bw6761.Ntt(b_device, core.KForward, &cfg, b_device)
	bw6761.Ntt(c_device, core.KForward, &cfg, c_device)
	...
	// h = ifft_coset(ca o cb - cc)
	bw6761.VecOp(a_device, b_device, a_device, vcfg, core.Mul)
	bw6761.VecOp(a_device, c_device, a_device, vcfg, core.Sub)
